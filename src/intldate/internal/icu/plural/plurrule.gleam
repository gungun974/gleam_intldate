...
import intldate/internal/icu/icudata/resource.{
  type Constraint, type NumRange, type PluralRule, Constraint, NumRange,
}
...
pub fn parse_condition(
  rule_data: String,
) -> Result(Option(List(List(resource.Constraint))), String) {
...
pub fn eval_condition(
  condition: List(List(resource.Constraint)),
  number: PluralOperands,
) -> Bool {
  case condition {
    [] -> False
...
fn find_rule_set(
  locales_table: dict.Dict(String, String),
  name: String,
) -> Option(String) {
  case dict.get(locales_table, name) {
    Ok(rule_set) -> Some(rule_set)
    Error(_) ->
      case bundle.chop_locale(name) {
        None -> None
        Some(next_name) -> find_rule_set(locales_table, next_name)
      }
  }
}
...
pub fn get_rule_from_resource(
  ...
  let locales_table = case type_ == uplural_type_ordinal {
    True -> plurals.locales_ordinals
    False -> plurals.locales
  }

  case find_rule_set(locales_table, name) {
    None -> []
    Some(rule_set) -> {
      case dict.get(plurals.rules, rule_set) {
        Ok(rules) -> rules
        Error(_) -> []
      }
    }
