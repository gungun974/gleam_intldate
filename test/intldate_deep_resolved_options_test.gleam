...
import intldate_test_data.{
  type ResolvedOptionsTestCase, ExpectOptions, ExpectOptionsError,
}
import simplifile
...
pub fn resolved_options_json_cases_test_() {
  use <- Timeout(timeout)
  let assert Ok(content) = simplifile.read("test/resolved_options.json")
  let assert Ok(cases) =
    json.parse(
      content,
      decode.list(intldate_test_data.resolved_options_test_case_decoder()),
    )

  let failures = cases |> list.filter_map(run_options_case)

  case failures {
    [] -> Nil
    failures -> panic as report("intldate resolved_options.json", failures)
  }
}

@target(javascript)
pub fn resolved_options_json_cases_test() {
  resolved_options_json_cases_test_().function()
}
