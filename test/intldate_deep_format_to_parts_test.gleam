...
import intldate_test_data.{
  type FormatPartsTestCase, ExpectParts, ExpectPartsError,
}
import simplifile
...
pub fn format_parts_json_cases_test_() {
  use <- Timeout(timeout)
  let assert Ok(content) = simplifile.read("test/format_parts.json")
  let assert Ok(cases) =
    json.parse(
      content,
      decode.list(intldate_test_data.format_parts_test_case_decoder()),
    )

  let failures = cases |> list.filter_map(run_parts_case)

  case failures {
    [] -> Nil
    failures -> panic as report("intldate format_parts.json", failures)
  }
}

fn run_parts_case(test_case: FormatPartsTestCase) -> Result(Failure, Nil) {
  case timestamp.parse_rfc3339(test_case.date) {
    Error(_) ->
      Ok(Failure(
        file: "format_parts.json",
